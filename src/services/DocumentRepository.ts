...
const nanoid = customAlphabet(
  '1234567890abcdef0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ',
  12
);
...
export default class DocumentRepository {
  ...
  async putContentAsync(
    index: { contentType: DocumentContentType; id?: string; description: string },
    content: any
  ): Promise<string> {
    ...
    if (this.contentClient.bucketName === undefined)
      throw new Error('this.contentClient.bucketName === undefined');
  ...
  async getIndexByS3Async(s3BucketName: string, s3Key: string): Promise<DocumentIndex> {
    ...
    const indexes = await this.indexClient.queryByIndexAsync<DocumentIndex>(
      'S3Key',
      'index',
    ...
    if (indexes.length !== 1) {
      throw new Error(
        `Found ${indexes.length} indexes when 1 was expected: ${JSON.stringify({
          s3BucketName,
          s3Key,
        })}`
      );
  ...
  async getContentAsync<T extends object>(
  ...
  private async listIndexesByDocumentTypeAsync(
    contentType: DocumentContentType
  ): Promise<DocumentIndex[]> {
    if (this.indexClient === undefined) throw new Error('this.metadataClient === undefined');
    const indexesByDocumentType = await this.indexClient.queryByPartitionKeyAsync<DocumentIndex>(
      contentType
    );
